...
public class IdentidySelectedColumns implements BasePatternDetector {
    ...
    static void checkTable(SelectedColumnsVisitor.TableWithSelectedCol tableWithSelectedCol, SimpleCatalog catalog, ArrayList<String> result) throws NotFoundException {
        ...
        if(tableInCatalog.getColumnCount() == tableWithSelectedCol.getSelectedColumns().size()) {
           result.add(String.format("All columns on table: %s are being selected. Please be sure that all columns are needed", tableName));
        }
