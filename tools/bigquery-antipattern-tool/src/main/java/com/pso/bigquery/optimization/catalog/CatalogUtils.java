...
public class CatalogUtils {
  ...
  private static void addTableToCatalog(SimpleCatalog catalog, SimpleTable table) {
  ...
  public static void createTableInCatalog(
      SimpleCatalog catalog, String tableName, List<SimpleColumn> columns) {
    CatalogUtils.addTableToCatalog(catalog, new SimpleTable(tableName, columns));
  }
  ...
  // For context, I recommend reading:
  // https://github.com/google/zetasql/issues/108#issuecomment-1128139006
  public static void createTableInCatalog(
    ...
    //      * Nested table with project and dataset merged (catches `project.dataset`.table, notice
    // quotes)
    ...
    SimpleCatalog datasetWithProject =
        CatalogUtils.getOrCreateNestedCatalog(catalog, datasetWithProjectId);
  ...
  private static void addFunctionToCatalog(
    ...
    Function function = new Function(namePath, group, mode, List.of(signature));
  ...
  public static void createFunctionInCatalog(
    ...
    CatalogUtils.addFunctionToCatalog(catalog, List.of(joinedNamePath), group, mode, signature);

    if (namePath.size() > 1) {
      String nestedCatalogName = namePath.get(0);
      SimpleCatalog nestedCatalog =
          CatalogUtils.getOrCreateNestedCatalog(catalog, nestedCatalogName);

      List<String> nestedNamePath = namePath.subList(1, namePath.size());
      CatalogUtils.createFunctionInCatalog(nestedCatalog, nestedNamePath, group, mode, signature);
    }
  }
